...
import { saveLegacy, type LegacyEntry } from '@/lib/legacy-storage';

interface EmotionData { id: string; name: string; emoji: string; intensity: 1 | 2 | 3; }
interface WhisperData { message: string; scripture: string; reference: string; character: string; }
interface ObedienceData { action: string; description: string; }
interface CompanionData { name: string; emoji: string; color: string; }
...
export default function LegacyPage() {
  ...
  useEffect(() => {
    ...
    setData({ emotion, intensity: emotion.intensity || 1, companion, prayer, whisper, obedience });
    setIsLoading(false);
  }, []);
  ...
  const handleSave = async () => {
    ...
    const entry: LegacyEntry = {
      id: `legacy_${Date.now()}`,
      emotion: { id: data.emotion.id, name: data.emotion.name, emoji: data.emotion.emoji },
      ...
      obedience: { action: data.obedience.action || '', description: data.obedience.description || '' },
      testimony,
      created_at: new Date().toISOString(),
    };

    const result = await saveLegacy(entry);

    if (result.ok) {
      // sessionStorage 정리
      ['chapter0_emotion','chapter0_prayer','chapter0_whisper','chapter0_obedience','chapter0_companion']
    ...
    <div className="min-h-screen bg-dark-navy p-6 flex items-center justify-center">
      <div className="text-center max-w-md">
        ...
        <p className="text-cream mb-8 leading-relaxed">
          당신의 여정이 기록되었습니다.<br /><br />THE WAY가 살아났습니다.
        </p>
  ...
  if (saveState === 'error') return (
    <div className="min-h-screen bg-dark-navy p-6 flex items-center justify-center">
      <div className="text-center max-w-md">
        ...
        <p className="text-sub-text mb-8">잠시 후 다시 시도해주세요.</p>
        <button onClick={() => setSaveState('idle')} className="bg-gold text-dark-navy px-8 py-3 rounded-lg font-bold hover:bg-yellow-500">
          다시 시도
        </button>
      </div>
    </div>
  ...
  return (
    <div className="min-h-screen bg-dark-navy p-6 flex flex-col items-center justify-center">
      <div className="max-w-2xl w-full">
        <h1 className="text-2xl font-bold text-cream mb-2 text-center">당신의 기도와 깨달음을 남겨주세요</h1>
        <p className="text-sub-text text-center mb-8 text-sm">이 순간의 간증을 기록하세요.</p>
        ...
        <div className="bg-emotion-purple rounded-lg p-6 mb-8 border border-gold space-y-4">
          <div className="grid grid-cols-2 gap-4 text-sm">
            <div>
              ...
              <p className="text-cream font-bold">{data?.emotion?.name} {data?.emotion?.emoji}</p>
            </div>
            <div>
              <p className="text-xs text-sub-text mb-1">강도</p>
              <p className="text-cream font-bold">
                {['한 스푼','두 스푼','세 스푼'][(data?.intensity ?? 1) - 1]}
              </p>
            </div>
          </div>
          {data?.companion && (
          ...
          {data?.whisper?.reference && (
            <div>
              ...
              <p className="text-gold text-xs font-bold">{data.whisper.reference} — {data.whisper.character}</p>
            </div>
          )}
        ...
        <textarea
          ...
          placeholder="이 경험 속에서 느낀 것, 깨달은 것을 기록하세요. (선택 사항)"
        ...
        />
        <div className="text-right text-sub-text text-xs mb-8">{testimony.length} / 1000자</div>

        <div className="flex justify-center gap-4">
          <button onClick={() => router.back()} className="bg-dark-blue text-cream px-8 py-3 rounded-lg font-bold border border-gold hover:border-yellow-500 transition-colors">
            다시 보기
          </button>
          <button onClick={handleSave} className="bg-gold text-dark-navy px-8 py-3 rounded-lg font-bold hover:bg-yellow-500 transition-colors flex-1">
            기록 저장
          </button>
        </div>
        <p className="text-center text-sub-text text-xs mt-6">기록은 클라우드에 안전하게 저장됩니다.</p>
      </div>
    </div>
  );
}
